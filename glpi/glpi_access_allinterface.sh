...
# 4) Aktifkan session.cookie_httponly
...
# 9.1) Bind MariaDB ke semua interface
log "Configure MariaDB bind-address → 0.0.0.0"
sed -i "s/^bind-address.*/# &/" /etc/mysql/mariadb.conf.d/50-server.cnf
echo "bind-address = 0.0.0.0" >> /etc/mysql/mariadb.conf.d/50-server.cnf
# restart MariaDB dengan config baru
pkill mysqld_safe mysqld || true
mysqld_safe --datadir=/var/lib/mysql --pid-file=/run/mysqld/mysqld.pid &
sleep 5

# 9.2) Grant network access
log "Grant network access to glpiuser"
mysql -uroot <<SQL
GRANT ALL PRIVILEGES ON glpi.* 
  TO 'glpiuser'@'%' IDENTIFIED BY 'glpipass';
FLUSH PRIVILEGES;
SQL

# 10) Buat DB & user GLPI untuk localhost
